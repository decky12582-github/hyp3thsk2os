class PurchaseWindow extends TabbedPopupWindow
{
    ...
    constructor(boundingBox, worldIndex = 0)
    {
        ...
        if(getBuildTarget() == STEAM_BUILD || platform.domain == "armorgames")
        {
            if(shopVariantId == 0)
            {
            ...
            },
            {
                onmousedown: function ()
                {
                    if(tickets > 9)
                    {
                        ...
                        chestService.spawnChest(0, Chest.purchased, true);
                        chestService.presentChest(0);
    ...
    onTabChange()
    {
        ...
        {
            this.buyButtons[i].setVisible(this.currentTabIndex == 0)
            this.buyButtons[i].setEnabled(this.currentTabIndex == 0)
        }
        for(var i in this.useButtons)
        {
            this.useButtons[i].setVisible(this.currentTabIndex == 1)
            this.useButtons[i].setEnabled(this.currentTabIndex == 1)
        }
    }
    ...
    render()
    {
        ...
        else
        {
            ...
            this.context.shadowOffsetX = 0;
            this.context.shadowOffsetY = 0;
            PU.drawImage(chest1, 0, 0, chest1.width, chest1.height, purchasedw * .15, purchasedh * .2, purchasedw * .3, purchasedh * .48);
            PU.drawImage(chest2, 0, 0, chest2.width, chest2.height, purchasedw * .55, purchasedh * .2, purchasedw * .3, purchasedh * .48);
            PU.font = "24px KanitM";
            PU.fillStyle = "#1798c7";
            PU.fillText(_("BUY"), purchasedw * .15 + (purchasedw * .3 / 2) - (PU.measureText(_("BUY")).width / 2), purchasedh * .2 + purchasedh * .42);
            PU.fillText(_("BUY"), purchasedw * .55 + (purchasedw * .3 / 2) - (PU.measureText(_("BUY")).width / 2), purchasedh * .2 + purchasedh * .42);
            PU.drawImage(smallShopTicket, 0, 0, smallShopTicket.width, smallShopTicket.height, purchasedw * .45, purchasedh * .85, purchasedw * .09, purchasedh * .05);
            PU.fillStyle = "#FFF";
            PU.fillText("x" + tickets, purchasedw * .55, purchasedh * .89);
        }
