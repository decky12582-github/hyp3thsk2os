class CaveWindow extends TabbedPopupWindow
{
    ...
    initializeCaveDisplay()
    {
        ...
        for(var i = 0; i < nodesInSystem.length; i++)
        {
            ...
            {
                var cave = parentWindow.caveSystem;
                if(!parentWindow.droneMenu.isVisible())
                {
                    this.highlightColor = "#76E374";
                    if(numberOfCavesExplored <= 1 && parentWindow.caveSystem.currentFuel >= 100)
                    {
                        return node.children.length == 0 || (node.isRevealed && cave.getRewardsOnNode(node).length > 0);
                    }
                }
    ...
    initializeDroneMenu()
    {
        ...
        for(var i = 0; i < blueprintList.length; ++i)
        {
            ...
            itemHitbox.isEnabled = () => this.droneMenu.selectedBlueprint == null;
            container.addHitbox(itemHitbox, true);

            var hintHighlight = new EasyHintHighlight(function (parentWindow, i)
            {
                var cave = parentWindow.caveSystem;
                if(numberOfCavesExplored <= 1)
                {
                    return (i == 2 && cave.currentFuel >= 200 && parentWindow.caveNode.children.length == 0) ||
                        (i == 0 && cave.currentFuel >= 100 && (parentWindow.caveNode.children.length > 0 || cave.currentFuel < 200))
                }
                return false;
    ...
    initializeDroneCraftingScreen()
    {
        ...
        var hintHighlight = new EasyHintHighlight(() => numberOfCavesExplored <= 1 && this.caveSystem.canCraftDrone(this.droneMenu.selectedDrone));
        hintHighlight.lineWidth = 0;
        hintHighlight.highlightBlur = 20;
        hintHighlight.sizeReduction = 8;
        hintHighlight.fillRelativeAlpha = 1;
        sendDroneButton.addHitbox(hintHighlight);
    }
