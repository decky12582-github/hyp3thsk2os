...
class MainUILayer extends UiLayer
{
    ...
    renderHeader()
    {
        ...
        if(capacity > 0)
        {
            if(isCapacityFull())
            {
                ...
                if(numFramesRendered % 100 == 1 && playtime < 7200)
                {
        ...
        framesRendered2++;
        if(framesRendered2 % 2 == 1 && isCapacityFull())
        {
    ...
    renderRightSideBar()
    {
        ...
        this.context.drawImage(settingsb, 0, 0, 33, 34, Math.ceil(mainw * .937), Math.ceil(mainh * .115), Math.floor(mainw * .0305), Math.ceil(mainh * .058));

        var y0 = Math.ceil(mainh * 0.175);
        var yIncrement = Math.ceil(mainh * 0.06);
        if(!isCraftNotificationOn)
        {
            this.context.drawImage(hammerAnvil, 0, 0, hammerAnvil.width, hammerAnvil.height, Math.ceil(mainw * .937), y0, Math.floor(mainw * .0305), Math.ceil(mainh * .058));
        }
        else
        {
        ...
        if(depth >= tradeConfig.tradingPosts[0].depth)
        {
            ...
            {
                this.context.drawImage(tradingpostb, 0, 0, 33, 39, Math.ceil(mainw * .937), y0 + yIncrement, Math.floor(mainw * .0305), Math.ceil(mainh * .058));
            }
            else
            {
                ...
                this.context.drawImage(icon, Math.ceil(mainw * .937), y0 + yIncrement);
            }
        }
        if(hasUnlockedScientists)
        {
            ...
            var iconY = y0 + yIncrement * 3;
            var iconWidth = Math.floor(mainw * .0305);
            var iconHeight = Math.ceil(mainh * .058);
            var dotWidth = iconWidth / 6;
            this.context.save();
            this.context.drawImage(scientistb, iconX, iconY, iconWidth, iconHeight);
            for(var i in activeScientists)
            {
                if(activeScientists[i].length > 0)
                {
                    ...
                    this.context.arc(dotX, dotY, dotWidth / 2, 0, Math.PI * 2);
                    this.context.fill();
                    this.context.stroke();
                }
            }
        ...
        if(depth >= 304)
        {
            ...
            var dotColor;
            var iconX = Math.ceil(mainw * .937);
            var iconY = y0 + yIncrement * 4;
            var iconWidth = Math.floor(mainw * .0305);
            var iconHeight = Math.ceil(mainh * .058);
            var dotWidth = iconWidth / 6;
            this.context.save();
            this.context.drawImage(undergroundb, iconX, iconY, iconWidth, iconHeight);
        ...
        if(depth >= 501)
        {
            this.context.drawImage(coreb, Math.ceil(mainw * .937), y0 + yIncrement * 5, Math.floor(mainw * .0305), Math.ceil(mainh * .058));
        }

        //Buffs
        // for(var i = 0; i < buffs.numActiveBuffs(); i++)
        // {
        //     renderBuff(this.context, Math.ceil(mainw * .902), Math.ceil(mainh * .11) + (Math.ceil(mainh * .06) * i), Math.floor(mainw * .03), Math.ceil(mainh * .055), i);
        // }
        ...
        if(depth >= tradeConfig.tradingPosts[1].depth)
        {
            ...
            {
                this.context.drawImage(lunartradingpostb, 0, 0, 33, 39, Math.ceil(mainw * .937), y0 + yIncrement * 6, Math.floor(mainw * .0305), Math.ceil(mainh * .058));
            }
            else
            {
                ...
                this.context.drawImage(icon, Math.ceil(mainw * .937), y0 + yIncrement * 6);
            }
        }

        if(depth >= 1334)
        {
            this.context.drawImage(reactorb, Math.ceil(mainw * .937), y0 + yIncrement * 7, Math.floor(mainw * .0305), Math.ceil(mainh * .058));
        ...
        if(depth >= CAVE_BUILDING_DEPTH)
        {
            var frameNum = numFramesRendered % 8;
            var iconX = Math.ceil(mainw * .937);
            var iconY = y0 + yIncrement * 2;
            var iconWidth = Math.floor(mainw * .0305);
            var iconHeight = Math.ceil(mainh * .058);
            var dotWidth = iconWidth / 6;
            if((frameNum == 0 || frameNum == 2) && treasureStorage.isFull())
            {
                if(!this.redCaveButtonIcon)
                {
                    var tempCanvas = document.createElement('canvas');
                    var tempContext = tempCanvas.getContext('2d');
                    tempCanvas.width = caveb.width;
                ...
                this.context.drawImage(this.redCaveButtonIcon, iconX, iconY, iconWidth, iconHeight);
            }
            else
            {
                this.context.drawImage(caveb, iconX, iconY, iconWidth, iconHeight);
            }
            var dotColor = "#888888";
            for(var i in caves)
            {
    ...
    renderPopupDialogueAttachment()
    {
        if(this.activeDialogue.x > 0)
        {
            if(activeLayers.hasOwnProperty(this.activeDialogue.popupId))
            {
                ...
                var dialogueImage;
                if(this.activeDialogue.extraImage && numFramesRendered % CHARACTER_BLINK_PERIOD == 0)
                {
