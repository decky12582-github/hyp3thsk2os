...
function renderFancyProgressBar(context, progressBarText, fillPercent, x, y, width, height, fillColor, emptyColor, fontColor, frameImage, drawGradient = true, frameImageInfo = null)
{
    ...
    context.fillRect(x + 1, y + 1, width * fillPercent - 2, height - 2);
    if(drawGradient)
    {
...
function drawImageFitInBox(context, image, x, y, maxWidth, maxHeight)
{
    var scale = 1;
    var xOffset = 0;
    var yOffset = 0;
    var verticalScale = maxHeight / image.height;
    if(image.width * verticalScale >= maxWidth)
    {
        // If scaling based on height makes it too wide, use width instead
        scale = maxWidth / image.width;
        xOffset = 0;
        yOffset = maxHeight / 2 - (image.height * scale) / 2;
    }
    else
    {
        scale = verticalScale;
        xOffset = maxWidth / 2 - (image.width * scale) / 2;
        yOffset = 0;
    }
    // console.log(image);
...
function drawBlessedEffect(canvas, image, canvasX, canvasY, canvasW, canvasH, relicName = "")
{
    if(quality == 1)
    {
        ...
        for(var i = 2; i < 36; i++)
        {
            var oscillationPeriod = 7;
            var oscillationNumber = Math.floor(((numFramesRendered % 3000) + i) / (oscillationPeriod * 2));
            ...
            canvas.globalAlpha = 1 - oscillate(((numFramesRendered % 3000) + i), oscillationPeriod);
    ...
    if(relicName != "???")
    {
        ...
        var shadowAlpha = 0.75 + oscillate(numFramesRendered, 9) * .22;
        canvas.shadowColor = "rgba(255, 255, 64, " + shadowAlpha + ")";
    }
    canvas.filter = 'brightness(' + (180 + (oscillate(numFramesRendered, 20) * 60)) + '%) saturate(200%)';
    ...
    if(numFramesRendered % 16 < 4)
    {
        ctx.globalAlpha = 0.5;
        ctx.drawImage(shineOverlay, (shineOverlay.width / 4) * (numFramesRendered % 4), 0, Math.floor(shineOverlay.width / 4), shineOverlay.height, 0, 0, canvasW, canvasH);
        ctx.globalAlpha = 1;
    }
...
function renderChestSliderDots()
{
    ...
    if(metalDetectorStructure.level > 1)
    {
        chestService.forEachChest((chest, tenthOfDepth) =>
        {
            ...
            if(chest.isGolden && metalDetectorStructure.level > 2)
            {
                chestDot = sliderg;
            }
            ...
            MAIN.drawImage(chestDot, 0, 0, 6, 10, Math.ceil(mainw * .0544), Math.ceil(mainh * (.3165 + (.490 * (Math.min(tenthOfDepth * 10 + chest.depthInBlock + 2, depth) / depth)))) - (Math.ceil(mainh * .007)), Math.floor(mainw * .0048), Math.ceil(mainh * .007));
            if(metalDetectorStructure.level > 4)
            {
                MAIN.globalAlpha = 1;
            }
