...
function clickedGap(relativeDepth, gapNumber)
{
    ...
    var tenthOfDepthClicked = Math.floor(depthClicked / 10);
    if(metalDetectorStructure.level >= 5 && chestService.chestExists(tenthOfDepthClicked) && !isClickableAtDepth(depthClicked))
    {
        chestService.presentChest(tenthOfDepthClicked);
    }
}
...
{
    if(depth > 100 && clickableRoller.rand(0, 30) == 0 && numClickablesOfType(MINERAL_DEPOSIT_ID) < MAX_SPAWNED_MINERAL_DEPOSITS && !isCapacityFull()) //every 15min on average, no more than 3 available at a time
    {
