...
function getRewardDescription(id, scientistIndex = null)
{
    ...
    if(excavationRewards[id].isRelic)
    {
        ...
        {
            description += "<br></br>" + excavationRewards[id].statType.getTooltip(excavationRewards[id].amount);
        }
    }
    else
    {
...
function getCostToResurrect(scientistIndex)
{
    ...
    var activeScientist = activeScientists[scientistIndex];
    var staticScientist = scientists[activeScientist[0]];
    var scientistRarityRessurectionMultiplier = 2 - staticScientist.deathChanceMultiple;

    return Math.round(Math.pow(scientistLevel, .75) * scientistRarityRessurectionMultiplier);
}
...
function progressScientistExcavations(seconds)
{
    ...
    {
        if(activeExcavations[i].length > 0 && !isExcavationDone(i) && !isScientistDead(i))
        {
            activeExcavations[i][2] -= seconds;

            if(isExcavationDone(i))
            {
                trackEvent_FinishExcavation(1);
            } else if(isScientistDead(i))
            {
...
{
    chestService.spawnChest(0, Chest.excavation, true);
    chestService.presentChest(0);
}
