...
var cachedMineralsPerSecondDepth = 0;
...
function runOptimizedMiningForOneMinute()
{
    if(!isCapacityFull())
    {
        ...
        for(var index in mineralsPerSecond)
        {
            var rewardAmount = randRoundToInteger(mineralsPerSecond[index] * 60);
            worldResources[index].numOwned += rewardAmount;
            capacity += rewardAmount;
        }
    }
}

function estimatedMineralsPerMinuteAtLevel(depthToEstimate)
{
    ...
    var results = [];
    var findMultiplier = 1;
    for(y = 0; y < levels[depthToEstimate].length; y++)
    {
        if(levels[depthToEstimate][y][0] < 20)
        {
            findMultiplier = STAT.isotopeFindChanceMultiplier();
        } //isotope
        ...
        findMultiplier *= STAT.minerSpeedMultiplier(); //(speed up mining by 5%)
        var threshold = levels[depthToEstimate][y][1] * findMultiplier;
        var averageResources = threshold / 1000;
        results[levels[depthToEstimate][y][0]] = (averageResources * chanceOfMiningLevel * 600);
    }
    return results;
}

//Copy pasta due to optimization requirements
function estimateTotalMineralsMinedPerSecond()
{
    var results = [];
    var findMultiplier = 1;
...
function valueOfMineralsPerSecond()
{
...
function getMineralSaleStats(x)
{
    ...
    let lockedAmount = lockedMineralAmtsToSave[x];
...
{
    var levelsTowardNextUpgrade = drillState.levelsTowardNextUpgrades[CARGO_TYPE];
    var nextUpgrade = getDrillEquipByLevel(CARGO_TYPE, drillState.cargo().level + 1);

    if(nextUpgrade && levelsTowardNextUpgrade < 10)
    {
        return (drillState.cargo().capacity + (((nextUpgrade.capacity - drillState.cargo().capacity) * .05) * levelsTowardNextUpgrade)) * STAT.cargoCapacityMultiplier();
    }
    else if(nextUpgrade && levelsTowardNextUpgrade >= 10)
    {
        return nextUpgrade.capacity * STAT.cargoCapacityMultiplier();
    }
    else
    {
...
function getUsedMineralCapacity()
{
    ...
    {
        if(worldResources[i].countsTowardsCapacityAndValue)
        {
            capacity += parseInt(worldResources[i].numOwned);
        }
    }
...
{
    return worldResources[mineralId].name;
}
