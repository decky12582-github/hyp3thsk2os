...
document.addEventListener("keydown", e =>
{
	...
	if(metalDetectorStructure.level >= 4)
	{
		...
		{
			//Chests
			for(var i = 0; i < chestService.chests.length; i++)
			{
				if(chestService.chests[i])
				{
					panToViewDepth(chestService.chests[i].tenthOfDepth * 10.0 + chestService.chests[i].depthInBlock)
					return;
				}
			}
