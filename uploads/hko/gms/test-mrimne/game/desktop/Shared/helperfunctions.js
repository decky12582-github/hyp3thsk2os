...
function fillTextWrapWithHeightLimit(context, text, x, y, maxWidth, maxHeight, justify = "left", lineSpacing = 0.5, verticalJustify = "top")
{
	if(debugShrunkText)
	{
		context.save();
	...
	{
		var result = fillTextWrap(context, text, x, y, maxWidth, justify, lineSpacing, false);
	}
	else
	{
		...
		{
			var result = fillTextWrap(context, text, x, y + extraVerticalSpace, maxWidth, justify, lineSpacing, false);
		}
		else if(verticalJustify == "center")
		{
			var result = fillTextWrap(context, text, x, y + (extraVerticalSpace/2), maxWidth, justify, lineSpacing, false);
		}
	}
	context.restore();
	return result;
...
function fillTextShrinkToFit(context, text, x, y, maxWidth, justify = "left", stretchTextAmount = 0)
{
	if(debugShrunkText)
	{
		context.save();
...
{
	window.open(_("https://discord.gg/C3KwWQr"), '_blank');
}

function openTwitter()
{
	window.open(_("https://twitter.com/MrMinecomgame"), '_blank');
}
...
function takeScreeshotForLanguage()
{
	...
	takeScreenshot(language + "_" + currentTime() + ".png").then(() => {takeScreeshotForLanguage();});
...
function takeScreenshot(fileName)
{
	...
	return win.webContents
		...
		.then((img) =>
		{
			fs.writeFile("../Screenshots/" + fileName,
