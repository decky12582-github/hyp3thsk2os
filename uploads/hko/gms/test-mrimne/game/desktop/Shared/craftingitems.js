...
class MineralCraftingItem extends CraftingItem
{
    ...
    getName()
    {
        if(worldResources[this.id].isIsotope)
        {
            ...
            {
                return _(worldResources[this.id].name);
            }
        }
        else if(worldResources[this.id].isOnHeader)
        {
            if(this.id > highestOreUnlocked)
            {
                return _("???");
...
class StructureCraftingItem extends CraftingItem
{
    ...
    getDescription()
    {
        ...
        if(this.getCurrentLevel() >= this.getMaxLevel())
        {
            ...
            description += structures[this.id].leveledStatName + ": " + structures[this.id].statValueForCurrentLevel();
        }
        else
        {
            var nextLevel = this.getCurrentLevel() + 1;
            if(this.getCurrentLevel() == 0)
            {
                ...
                description += structures[this.id].leveledStatName + ": " + structures[this.id].statValueForNextLevel();
            }
            else if(nextLevel)
            {
                description += _("Level: {0}", currentLevel) + String.fromCharCode(8201) + String.fromCharCode(8594) + String.fromCharCode(8201) + nextLevel + " <br> ";
                description += structures[this.id].leveledStatName + ": " + structures[this.id].statValueForCurrentLevel() + String.fromCharCode(8201) + String.fromCharCode(8594) + String.fromCharCode(8201) + structures[this.id].statValueForNextLevel();
            }
        }
        return description;
    }
