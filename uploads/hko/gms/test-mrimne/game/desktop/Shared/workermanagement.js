...
function onWorkerClicked(d, workerNum)
{
    ...
    var tenthDepth = Math.floor(depthToCheck / 10);

    if(chestService.chestExistsAtDepth(depthToCheck))
    {
        if(chestService.getChest(tenthDepth).worker == workerNum || metalDetectorStructure.level >= 5)
        {
            chestService.presentChest(tenthDepth);
        }
        else
        {
