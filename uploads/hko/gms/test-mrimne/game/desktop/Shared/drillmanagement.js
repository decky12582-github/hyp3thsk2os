...
class DrillState
{
    ...
    statForType(type, levelsTowardNextUpgrade, stat)
    {
        ...
        var currentMultiplier = doBigIntDecimalMultiplication(currentUpgradeStat, percentOfMultiplierFromCurrentUpgrade);
    ...
    totalWattage()
    {
        return this.totalBaseWattage() * this.totalWattMultiplier();
    }
}
...
function dig()
{
    if(progressTowardsNextDepth >= depthDifficultyTable[depth] && !isWaitingForLiftoff())
    {
    ...
    else
    {
        if(depth < (levels.length - 1) && !isStalledDueToBoss())
        {
            ...
            progressToAdd = doBigIntDecimalMultiplication(progressToAdd, STAT.drillSpeedMultiplier());
...
function subTickDistancePercent()
{
    ...
    var percentToNextTick = Math.min(1, ((currentTime() - lastTickTime) / 1000));
