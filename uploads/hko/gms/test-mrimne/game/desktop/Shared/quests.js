...
quest2.description = _("Share MrMine with a friend");
quest2.rewardType = REWARD_TYPE_TICKETS;
quest2.rewardAmount = 3;
quest2.moneyReward = 2500n;
quest2.isVisible = function () {return true;};
quest2.additionalOnClick = function () {showSimpleInput(_("Send this message to a friend to share the game with them!"), _("Copy Message"), generateShareText(), () => {copyShareText(); shareMouseDown();}, _("Cancel"));};
...
quest5.onComplete = function ()
{
    ...
    chestService.spawnChest(4, Chest.quest);
};
...
quest21.moneyReward = 100000000000n;
...
quest33.moneyReward = 1000000000000000n;
...
function hasLevel30Scientists()
{
    for(var i = 0; i < activeScientists.length; i++)
    {
        if(getScientistLevel(i) >= 30) return true;
    }
}
...
var shareCreditTimeSeconds = [302, 602, 1202]; //61 - 59(55), 121 - 50(49), 601 - 86(85)
var selectedCreditTimeSeconds = shareCreditTimeSeconds[rand(0, shareCreditTimeSeconds.length - 1)];

function shareMouseDown()
{
    if(getBuildTarget() != STEAM_BUILD) { } else
    {
        greenworks.activateGameOverlay("Friends");
    }
    setTimeout(function () {questManager.getQuest(1).isComplete = true;}, selectedCreditTimeSeconds * 1000);
}
...
function generateShareText()
{
    ...
    {
        var referUrl = "https://play.google.com/store/apps/details?id=com.playsaurus.mrmine";
        return _("Checkout this game I am playing {0}", referUrl);
    }
    else if(getBuildTarget() == WEB_BUILD)
    {
        var referUrl = "https://mrmine.com/?utm_campaign=WebRef&utm_content=" + selectedCreditTimeSeconds + "_" + getId(6);
        return _("Checkout this game I am playing {0}", referUrl);
    }
    else
    {
        return _("Checkout this game I am playing https://store.steampowered.com/app/1397920/MrMine")
    }
}
